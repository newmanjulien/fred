import { getActivityLevelLabel, getProbabilityLabel } from '$lib/dashboard/view-models/account';
...
import type { AccountKind } from '$lib/types/vocab';

const UPDATE_QUOTE_PERSON_KEYS = ['morgan-lee', 'taylor-shah'] as const;
const UPDATE_QUOTE_TEXT =
	'We prepared this insight using the data which was available, but we do not have the full picture. Upload anything we do not have access to and we will update the dashboard';
...
export function buildAccountHero(account: AccountHeroInput): CanvasHeroData {
	...
	const probabilityLabel = getProbabilityLabel(account.kind);
	const shouldShowStage = account.kind === 'new-business' && Boolean(account.stage);
	const accountSummary = shouldShowStage
		? `${account.accountName} is in ${account.stage} and is ${account.probability}% ${probabilityLabel} with ${activityLabel}.`
		: `${account.accountName} is ${account.probability}% ${probabilityLabel} with ${activityLabel}.`;

	return {
		accountNumber: account.accountNumber,
		title: account.accountName,
		description: `${accountSummary} ${account.context.summary}`
	};
}
